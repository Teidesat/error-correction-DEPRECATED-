...
DecoderScheme::DecoderScheme(int memorySize, int encoderOutputs, int encoderInputs)
    : memorySize(memorySize)
    , encoderOutputs(encoderOutputs)
    , encoderInputs(encoderInputs)
    , scheme()
{
    assert(memorySize >= 2 && memorySize <= 31); // Minimum k for the encoder is 2
    scheme.reserve(pow2(memorySize));

    for (uint32_t i = 0; i < pow2(memorySize); ++i) {
        Node node;
        Bit inAdder[2] = {0};

        inAdder[HIGH] = parity(i) XOR HIGH;
        inAdder[LOW ] = parity(i) XOR LOW;
        node.oneLink .dst = (i >> 1) | (static_cast<uint32_t>(inAdder[HIGH]) << (memorySize - 1));
        node.zeroLink.dst = (i >> 1) | (static_cast<uint32_t>(inAdder[LOW ]) << (memorySize - 1));
